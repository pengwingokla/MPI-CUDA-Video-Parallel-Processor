...
mkdir -p output/output_mpi_cuda/
...
mpirun -np 4 --allow-run-as-root\
    --output-filename output/output_mpi_cuda/logs ./bin/exec_full

echo "==> GENERATING VIDEO FROM PROCESSED FRAMES..."
ffmpeg -y -framerate 30 -i output/output_mpi_cuda/frame_%04d.jpg -c:v libx264 -pix_fmt yuv420p -crf 23 data/output/output_mpi_cuda.mp4
